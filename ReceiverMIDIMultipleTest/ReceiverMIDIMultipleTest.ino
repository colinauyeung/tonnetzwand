...
int buttonState = 0;
 ...
 void setup() {
  ...
  radio.openReadingPipe(0, addressBall);
...
void readWand()
{
  ...
  Serial.println("Looking for wand...");
  if (radio.available(addressWand)) {
    radio.read(&messageWand, sizeof(messageWand));

Serial.println("Wand available");
...
void readBall()
{
  //radio.openReadingPipe(0, addressBall);
  //radio.startListening();
  Serial.println("trying to read ball");

  if (radio.available(addressBall)) {
    radio.read(&messageBall, sizeof(messageBall));

    Serial.print(messageBall.ID); Serial.print(" ");
    Serial.print(messageBall.stateOne); Serial.print(" ");
    Serial.print(messageBall.stateTwo); Serial.print(" ");
    Serial.println();

    // Set ball actions
    if(messageBall.ID == 2)
    {
      gesture = messageBall.stateOne;
      buttonState = messageBall.stateTwo;
    }
  }
}
...
void setChordAction()
{
  ...
  // Determine if there has been a change in action
